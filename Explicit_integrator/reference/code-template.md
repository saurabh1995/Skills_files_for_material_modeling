...
class ConstitutiveMaterial(JAXMaterial):
    """
    Explicit integration scheme for viscoplastic material with damage.
    Based on radial return mapping algorithm (Algorithm 1).
    """
    ...
    def constitutive_update(self, eps, state, dt):
        ...
        Algorithm structure (from Algorithm 1):
        1. Extract old state (line 1)
        2. Elastic predictor (line 2-5)
        3. Check plasticity (line 6-7)
        4. Elastic or plastic branch (line 8-42)
        5. Update state variables (line 33-41)
        ...
        # ====================================================================
        # 1. EXTRACT OLD STATE VARIABLES (Algorithm 1, line 1)
        # ====================================================================
        ...
        # ====================================================================
        # 3. YIELD FUNCTION EVALUATION (Algorithm 1, line 7)
        # ====================================================================
        ...
        def _plastic_update(operand):
            ...
            # ================================================================
            # 6a. COMPUTE EQUIVALENT PLASTIC STRAIN RATE (Eq. 9)
            # ================================================================
            
            # Perzyna viscoplasticity: p_dot = <fy/K>^n
            # McCauley bracket <x> = max(x, 0) = 0.5*(x + |x|)
            x = fy / params.K_visc
            ...
            # ================================================================
            # 6b. COMPUTE FLOW DIRECTION (Eq. 8)
            # ================================================================
            
            # Flow direction: n = (sigma' - X') / J2(sigma - X)
            sig_eff = sig_trial - X_old
            sig_eff_dev = self._deviatoric(sig_eff)
            J2_eff = self._J2(sig_eff_dev)
            ...
            flow_dir = sig_eff_dev * inv_J2  # Normalized flow direction
            
            # ================================================================
            # 6c. COMPUTE INELASTIC STRAIN INCREMENT (Eq. 8)
            # ================================================================
            ...
            # Damage rate (Lemaitre damage, Eq. 7)
            sigma_eq = self._equiv_stress(sig_trial)  # Using trial stress
            ...
            D_new = jnp.clip(D_new, 0.0, params.Dc)  # Keep in [0, Dc]
            
            # Stress update with evolving damage
            sig_new = sig_old + (1.0 - D_new) * (C @ delta_eps_e)

            # ================================================================
            # 6f. UPDATE HARDENING VARIABLES (Eq. 10, 11)
            # ================================================================
            ...
            # ================================================================
            # 6g. RECOMPUTE YIELD FUNCTION FOR NEXT STEP
            # ================================================================
            
            # Use updated stress and hardening variables
            sig_eff_new = sig_new - X_new
            sig_eff_dev_new = self._deviatoric(sig_eff_new)
            J2_eff_new = self._J2(sig_eff_dev_new)
            sigma_eff_new = J2_eff_new / (1.0 - D_new)
            fy = sigma_eff_new - (R_new + params.k)
